...
import { ArrowLeft, Package, Plane, CheckCircle, Clock, AlertTriangle, MapPin } from "lucide-react";
...
export default function DetailShipment() {
  ...
  if (loading) {
    return (
      <div>
        <button onClick={() => router.push("/admin/shipments")} className="mb-4 flex items-center gap-2 text-blue-600 hover:text-blue-800 transition font-medium">
  ...
  const achievedStatuses = new Set(trackingHistory.map((t: any) => t.status));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-xl shadow p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <p className="font-semibold">{new Date(data.shipment_date).toLocaleDateString("en-GB")}</p>
            </div>
            <div>
          ...
          <div className="bg-white rounded-xl shadow p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <span className={`px-3 py-1 rounded-full text-xs font-medium ${getStatusColor(data.flight_status || "")}`}>
                  {data.flight_status || "—"}
                </span>
              </div>
            </div>
        ...
        {/* SENDER & RECEIVER */}
        <div className="bg-white rounded-xl shadow p-6">
          <h2 className="text-lg font-semibold text-blue-700 mb-4">Sender &amp; Receiver</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div><p className="text-gray-400 text-sm">Sender</p><p className="font-semibold">{data.sender_name}</p></div>
            <div><p className="text-gray-400 text-sm">Receiver</p><p className="font-semibold">{data.receiver_name}</p></div>
            <div><p className="text-gray-400 text-sm">Sender Phone</p><p className="font-semibold">{data.phone_number}</p></div>
            <div><p className="text-gray-400 text-sm">Receiver Phone</p><p className="font-semibold">{data.receiver_phone_number}</p></div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div><p className="text-gray-400 text-sm">Origin</p><p className="font-semibold">{data.origin_city}</p></div>
            <div><p className="text-gray-400 text-sm">Destination</p><p className="font-semibold">{data.destination_city}</p></div>
            <div><p className="text-gray-400 text-sm">Shipping Type</p><p className="font-semibold">{data.shipping_type}</p></div>
            <div><p className="text-gray-400 text-sm">Vehicle</p><p className="font-semibold">{data.vehicle_name}</p></div>
          </div>
        </div>

        {/* CARGO INFORMATION */}
        <div className="bg-white rounded-xl shadow p-6">
          <h2 className="text-lg font-semibold text-blue-700 mb-4">Cargo Information</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div><p className="text-gray-400 text-sm">Item Name</p><p className="font-semibold">{data.item_name}</p></div>
            <div><p className="text-gray-400 text-sm">Item Type</p><p className="font-semibold">{data.item_type}</p></div>
            <div><p className="text-gray-400 text-sm">Weight</p><p className="font-semibold">{data.weight} kg</p></div>
            <div><p className="text-gray-400 text-sm">Description</p><p className="font-semibold">{data.item_description || "—"}</p></div>
          </div>
        </div>

        {/* PAYMENT INFORMATION */}
        <div className="bg-white rounded-xl shadow p-6">
          <h2 className="text-lg font-semibold text-blue-700 mb-4">Payment Information</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div><p className="text-gray-400 text-sm">Shipping Price</p><p className="font-semibold">Rp {Number(data.shipping_price).toLocaleString("id-ID")}</p></div>
            <div><p className="text-gray-400 text-sm">Payment Method</p><p className="font-semibold">{data.payment_method}</p></div>
            <div><p className="text-gray-400 text-sm">Payment Date</p><p className="font-semibold">{new Date(data.payment_date).toLocaleDateString("en-US", { day: "numeric", month: "long", year: "numeric" })}</p></div>
            <div><p className="text-gray-400 text-sm">Transaction Status</p><p className="font-semibold">{data.transaction_status}</p></div>
          </div>
        </div>
